...
function __powerline_hostname_prompt() {
	...
	echo "${SHORT_HOSTNAME:-$(hostname -s)}|${HOST_THEME_PROMPT_COLOR?}|${fg_color}"
}
...
function __powerline_prompt_command() {
	...
	if [[ -n "${LEFT_PROMPT}" ]]; then
		LEFT_PROMPT+="$(set_color "${LAST_SEGMENT_COLOR}" -)${separator_char}${normal?}"
	fi
